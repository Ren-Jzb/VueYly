...
function getDel(k) {
    //第二行不删除，只清空
    var sum = $(k).closest("tr").index();
    if (sum == 1) {
        //alert($(k).parent().parent().find(".FoodName").html());
        $(k).parent().parent().find(".FoodName").html("");
        $(k).parent().parent().find(".Amount").html("");
    }
    else {
...
function DeleteRow(tb) {
    //var table = document.getElementById('tabSunday');
...
function AddRow(tb) {
    ...
    newRow.cells[0].innerHTML = lastRow.cells[0].innerHTML;
